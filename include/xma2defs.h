...
__inline DWORD GetXmaPacketFrameCount(_In_bytecount_(1) const BYTE* pPacket)
{
    return (DWORD)(pPacket[0] >> 2);
}

__inline DWORD GetXmaPacketFirstFrameOffsetInBits(_In_bytecount_(3) const BYTE* pPacket)
{
...
__inline DWORD GetXmaPacketMetadata(_In_bytecount_(3) const BYTE* pPacket)
{
    return (DWORD)(pPacket[2] & 0x7);
}

__inline DWORD GetXmaPacketSkipCount(_In_bytecount_(4) const BYTE* pPacket)
{
...
(
    ...
    _In_count_(nBlockCount) const DWORD* pSeekTable,  // Pointer to the seek table data
    DWORD nDesiredSample,                   // Decoded sample to locate
    _Out_ DWORD* pnBlockContainingSample,   // Index of the block containing the sample
    _Out_ DWORD* pnSampleOffsetWithinBlock  // Position of the sample in this block
)
{
...
(
    _In_bytecount_(nBitPosition / 8 + 3)
    _In_ const BYTE* pPacket,  // Pointer to XMA packet[s] containing the frame
...
(
    _In_bytecount_(nXmaDataBytes) const BYTE* pXmaData,  // Pointer to XMA block[s]
...
)
{
    ...
    pCurrentPacket = pXmaData + (DWORD_PTR)nStreamIndex * XMA_BYTES_PER_PACKET;
    for (;;)
    {
        ...
        pCurrentPacket += (DWORD_PTR)XMA_BYTES_PER_PACKET * (GetXmaPacketSkipCount(pCurrentPacket) + 1);
    }
}
...
)
{
    ...
    // fact that the first packets for each stream are in consecutive order at
    // the beginning of an XMA block.
    pLastPacket = pXmaData + (DWORD_PTR)nStreamIndex * XMA_BYTES_PER_PACKET;
...
(
    ...
    _Out_ DWORD* pnBitOffset,                         // Returns the bit offset within pXmaData of
                                                      // the frame containing the sample sought
    _Out_ DWORD* pnSubFrame                           // Returns the subframe containing the sample
)
{
...
__inline HRESULT LocalizeXma2Format(_Inout_ XMA2WAVEFORMATEX* pXma2Format)
{
